...
bool SliceMeshStorage::getExtruderIsUsed(const size_t extruder_nr) const
{
	...
	if(settings.get<bool>("anti_overhang_mesh") || layer_group.support_mesh)
	{
...
std::vector<RetractionConfig> SliceDataStorage::initializeRetractionConfigs()
{
	...
	ret.resize(Application::getInstance().scene.extruders.size()); // initializes with constructor RetractionConfig()
	return ret;
}
...
std::vector<WipeScriptConfig> SliceDataStorage::initializeWipeConfigs()
{
	...
	ret.resize(Application::getInstance().scene.extruders.size());
	return ret;
}
...
{
	std::vector<bool> ret;
	ret.resize(Application::getInstance().scene.extruders.size(), false);
	...
	if(mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") != EPlatformAdhesion::NONE)
	{
		...
		// process brim/skirt
		for(size_t extruder_nr = 0; extruder_nr < ret.size(); extruder_nr++)
		{
	...
	for(const SliceMeshStorage& mesh : meshes)
	{
		if(mesh.layer_group.support_enable || mesh.layer_group.support_tree_enable || mesh.layer_group.support_mesh)
		{
			...
			if(mesh_layer_group0.support_roof_enable)
