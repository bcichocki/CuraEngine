...
WallsComputation::WallsComputation(const Settings& settings, const Tina3D::LayerGroupSettings layer_group, const LayerIndex layer_nr) :
	settings(settings),
...
void WallsComputation::generateInsets(SliceLayerPart* part)
{
	...
	const bool recompute_outline_based_on_outer_wall = (layer_group.support_enable || layer_group.support_tree_enable) && !settings.get<bool>("fill_outline_gaps");
