...
template<> std::string Settings::get<std::string>(const std::string& key) const
{
	...
	//const auto& limit_to_extruder = Application::getInstance().scene.limit_to_extruder;

	//const auto&& ext_it = limit_to_extruder.find(key);

	//if(ext_it != limit_to_extruder.end())
	//	return ext_it->second->settings.getWithoutLimiting(key);
...
template<> std::string Settings::get<std::string>(const std::string& key, const std::string& def) const
{
	const auto&& it = settings.find(key);
	...
	if(parent)
		return parent->get<std::string>(key, def);
...
template<> char Settings::get<char>(const std::string& key, const std::string& def) const
{
	const auto&& it = settings.find(key);
	const char chr = def.empty() ? 'E' : def[0];

	if(it != settings.end())
		return it->second.empty() ? chr : it->second[0];

	if(parent)
		return parent->get<char>(key, def);
...
//template<> char Settings::get<char>(const std::string& key, const char& def) const
//{
//	const auto&& it = settings.find(key);
//
//	if(it != settings.end())
//		return it->second.empty() ? def : it->second[0];
//
//	if(parent)
//		return parent->get<char>(key, def);
//
...
template<> double Settings::get<double>(const std::string& key, const std::string& def) const
{
	return atof(get<std::string>(key, def).c_str());
}

//template<> double Settings::get<double>(const std::string& key, const double& def) const
//{
//	const auto&& it = settings.find(key);
//
//	if(it != settings.end())
...
//
//	if(parent)
//		return parent->get<double>(key, def);
//
//	logWarning("Trying to retrieve setting with no value given: '%s'\n", key.c_str());
//
//	return def;
...
template<> size_t Settings::get<size_t>(const std::string& key, const std::string& def) const
{
	return std::stoul(get<std::string>(key, def).c_str());
}

//template<> size_t Settings::get<size_t>(const std::string& key, const size_t& def) const
//{
//	const auto&& it = settings.find(key);
//
//	if(it != settings.end())
...
//
//	if(parent)
//		return parent->get<size_t>(key, def);
//
//	logWarning("Trying to retrieve setting with no value given: '%s'\n", key.c_str());
//
//	return def;
...
{	
	return to_bool(get<std::string>(key));
}

template<> bool Settings::get<bool>(const std::string& key, const std::string& def) const
{
	return to_bool(get<std::string>(key, def));
}

//template<> bool Settings::get<bool>(const std::string& key, const bool& def) const
//{
//	const auto&& it = settings.find(key);
//
//	if(it != settings.end())
//		return to_bool(it->second);
//
//	if(parent)
//		return parent->get<bool>(key, def);
//
//	logWarning("Trying to retrieve setting with no value given: '%s'\n", key.c_str());
//
//	return def;
...
template<> ExtruderTrain& Settings::get<ExtruderTrain&>(const std::string& key) const
{
	...
	return Application::getInstance().scene.extruders[extruder_nr];
}
...
template<> coord_t Settings::get<coord_t>(const std::string& key, const std::string& def) const
{
	return MM2INT(get<double>(key, def)); //The settings are all in millimetres, but we need to interpret them as microns.
}

//template<> coord_t Settings::get<coord_t>(const std::string& key, const coord_t& def) const
//{
//	const auto&& it = settings.find(key);
//
//	if(it != settings.end())
//		return MM2INT(atof(it->second.c_str()));
//
//	if(parent)
//		return parent->get<coord_t>(key, def);
//
//	logWarning("Trying to retrieve setting with no value given: '%s'\n", key.c_str());
//
//	return def;
...
template<> AngleRadians Settings::get<AngleRadians>(const std::string& key, const std::string& def) const
{
	return get<double>(key, def) * M_PI / 180; //The settings are all in degrees, but we need to interpret them as radians.
}
...
template<> AngleDegrees Settings::get<AngleDegrees>(const std::string& key, const std::string& def) const
{
...
template<> Temperature Settings::get<Temperature>(const std::string& key, const std::string& def) const
{
	return get<double>(key, def);
}
...
template<> Velocity Settings::get<Velocity>(const std::string& key, const std::string& def) const
{
	return get<double>(key, def);
}
...
template<> Ratio Settings::get<Ratio>(const std::string& key, const std::string& def) const
{
	return get<double>(key, def) / 100.0; //The settings are all in percentages, but we need to interpret them as radians.
}
...
template<> Duration Settings::get<Duration>(const std::string& key, const std::string& def) const
{
	return get<double>(key, def);
}
...
template<> EGCodeFlavor Settings::get<EGCodeFlavor>(const std::string& key) const
{
	...
	else if(value == "Tina3D")
		return EGCodeFlavor::TINA3D;
...
template<> EFillMethod Settings::get<EFillMethod>(const std::string& key, const std::string& def) const
{
	const std::string& value = get<std::string>(key, def);
...
const std::string Settings::getAllSettingsString() const
{
	...
	for(const auto& pair : settings)
		sstream << " -s " << pair.first << "=\"" << Escaped{pair.second.c_str()}.str << "\"";
