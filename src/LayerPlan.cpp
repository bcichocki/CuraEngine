...
static constexpr int MINIMUM_LINE_LENGTH = 5; // in uM. Generated lines shorter than this may be discarded
static constexpr int MINIMUM_SQUARED_LINE_LENGTH = MINIMUM_LINE_LENGTH * MINIMUM_LINE_LENGTH;

static constexpr float max_non_bridge_line_volume = 100000.0f; // limit to accumulated "volume" of non-bridge lines which is proportional to distance x extrusion rate
...
GCodePath* LayerPlan::getLatestPathWithConfig(const GCodePathConfig& config, SpaceFillType space_fill_type, const Ratio flow, const Ratio speed_factor)
{
	...
	if(!paths.empty())
	{
		...
		if(path.config == &config && !path.done && path.flow == flow && path.speed_factor == speed_factor && path.mesh_id == current_mesh) // spiralize can only change when a travel path is in between
			return &paths.back();
	}

	paths.emplace_back(config, current_mesh, space_fill_type, flow, current_group, speed_factor);
...
LayerPlan::LayerPlan(const SliceDataStorage& storage, LayerIndex layer_nr, coord_t z, coord_t layer_thickness, size_t start_extruder, const std::vector<FanSpeedLayerTimeSettings>& fan_speed_layer_time_settings_per_extruder, coord_t comb_boundary_offset, coord_t comb_move_inside_distance, coord_t travel_avoid_distance)
	...
	, layer_thickness(layer_thickness)
	, has_prime_tower_planned_per_extruder(Application::getInstance().scene.extruders.size(), false)
	, last_extruder_previous_layer(start_extruder)
	, last_planned_extruder(&Application::getInstance().scene.extruders[start_extruder])
	...
	, fan_speed_layer_time_settings_per_extruder(fan_speed_layer_time_settings_per_extruder)
{
	...
	if(Application::getInstance().scene.current_mesh_group->settings.get<CombingMode>("retraction_combing") != CombingMode::OFF)
	...
	extruder_plans.reserve(Application::getInstance().scene.extruders.size());
	...
	for(size_t extruder_nr = 0; extruder_nr < Application::getInstance().scene.extruders.size(); extruder_nr++)
	{
		//Skirt and brim.
...
{
	const CombingMode combing_mode = Application::getInstance().scene.current_mesh_group->settings.get<CombingMode>("retraction_combing");
...
bool LayerPlan::setExtruder(const size_t extruder_nr)
{
	...
	else
	{
		...
		assert(extruder_plans.size() <= Application::getInstance().scene.extruders.size() && "Never use the same extruder twice on one layer!");
	}

	last_planned_extruder = &Application::getInstance().scene.extruders[extruder_nr];
...
void LayerPlan::addExtrusionMove(Point p, const GCodePathConfig& config, SpaceFillType space_fill_type, const Ratio& flow, Ratio speed_factor, double fan_speed)
{
	GCodePath* path = getLatestPathWithConfig(config, space_fill_type, flow, speed_factor);
	path->points.push_back(p);
...
void LayerPlan::addPolygon(ConstPolygonRef polygon, int start_idx, const GCodePathConfig& config, WallOverlapComputation* wall_overlap_computation, coord_t wall_0_wipe_dist, const Ratio& flow_ratio, bool always_retract)
{
	...
	for(unsigned int point_idx = 1; point_idx < polygon.size(); point_idx++)
	{
		...
		addExtrusionMove(p1, config, SpaceFillType::Polygons, flow);
		p0 = p1;
	}
...
void LayerPlan::addPolygonsByOptimizer(const Polygons& polygons, const GCodePathConfig& config, WallOverlapComputation* wall_overlap_computation, const ZSeamConfig& z_seam_config, coord_t wall_0_wipe_dist, const Ratio flow_ratio, bool always_retract, bool reverse_order)
{
	if(polygons.size() == 0)
		return;
	...
	if(reverse_order == false)
	{
		...
		{
			addPolygon(polygons[poly_idx], orderOptimizer.polyStart[poly_idx], config, wall_overlap_computation, wall_0_wipe_dist, flow_ratio, always_retract);
		}
	}
	else
	{
...
{
	static constexpr coord_t min_line_len = 5; // we ignore lines less than 5um long
	static constexpr double acceleration_segment_len = 1000; // accelerate using segments of this length
	static constexpr double acceleration_factor = 0.75; // must be < 1, the larger the value, the slower the acceleration
			...
			if(distance_to_bridge_start > 0)
			{
				...
				if(coast_dist > 0 && ((distance_to_bridge_start - len) <= coast_dist))
				{
					if((len - coast_dist) > min_line_len)
					{
						...
						addExtrusionMove(segment_end + coast_dist * (cur_point - segment_end) / len, non_bridge_config, SpaceFillType::Polygons, segment_flow, speed_factor);
					}
					// then coast to start of bridge segment
					addExtrusionMove(segment_end, non_bridge_config, SpaceFillType::Polygons, 0, speed_factor);
				}
				else
				{
					// no coasting required, just normal segment using non-bridge config
					addExtrusionMove(segment_end, non_bridge_config, SpaceFillType::Polygons, segment_flow,
...
void LayerPlan::addWall(ConstPolygonRef wall, int start_idx, const SliceMeshStorage& mesh, const GCodePathConfig& non_bridge_config, const GCodePathConfig& bridge_config, WallOverlapComputation* wall_overlap_computation, coord_t wall_0_wipe_dist, float flow_ratio, bool always_retract)
{
	...
	for(unsigned int point_idx = 1; point_idx < wall.size(); point_idx++)
	{
		...
		if(flow >= wall_min_flow)
		{
			...
			if(is_small_feature)
			{
				addExtrusionMove(p1, non_bridge_config, SpaceFillType::Polygons, flow, small_feature_speed_factor);
			}
			else
			{
...
void LayerPlan::addLinesByOptimizer(const Polygons& polygons, const GCodePathConfig& config, SpaceFillType space_fill_type, bool enable_travel_optimization, int wipe_dist, float flow_ratio, std::optional<Point> near_start_location, double fan_speed)
{
	...
	for(unsigned int order_idx = 0; order_idx < orderOptimizer.polyOrder.size(); order_idx++)
	{
		...
		addTravel(p0);
		addExtrusionMove(p1, config, space_fill_type, flow_ratio, 1.0, fan_speed);
		...
		if(wipe_dist != 0)
		{
			...
			if(wipe)
				addExtrusionMove(p1 + normal(p1 - p0, wipe_dist), config, space_fill_type, 0.0, 1.0, fan_speed);
		}
	}
}
...
void LayerPlan::writeGCode(GCodeExport& gcode)
{
	...
	for(size_t extruder_plan_idx = 0; extruder_plan_idx < extruder_plans.size(); extruder_plan_idx++)
	{
		...
		if(extruder_nr != extruder_plan.extruder_nr)
		{
			...
			const ExtruderTrain& prev_extruder_train = Application::getInstance().scene.extruders[prev_extruder];
			...
			const ExtruderTrain& extruder = Application::getInstance().scene.extruders[extruder_nr];
		...
		else if(extruder_plan_idx == 0)
		{
			...
			else if(layer_nr != 0 && Application::getInstance().scene.extruders[extruder_nr].settings.get<bool>("retract_at_layer_change"))
			{
		...
		for(unsigned int path_idx = 0; path_idx < paths.size(); path_idx++)
		{
			...
			if(path.mesh_id != current_mesh)
			{
				...
				gcode.writeComment("MESH:" + current_mesh);
			}
			...
			const bool spiralize = path.spiralize;
...
{
	ExtruderPlan& extruder_plan = extruder_plans[extruder_plan_idx];
	const ExtruderTrain& extruder = Application::getInstance().scene.extruders[extruder_plan.extruder_nr];
