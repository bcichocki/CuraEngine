...
private:
	...
	{
		bool is_primed = false; //!< Whether this extruder has currently already been primed in this print

		bool is_used = false; //!< Whether this extruder train is actually used during the printing of all meshgroups
		char extruderCharacter = 0;

		double filament_area = 0; //!< in mm^2 for non-volumetric, cylindrical filament
		double totalFilament = 0; //!< total filament used per extruder in mm^3

		Temperature currentTemperature = 0;
		bool waited_for_temperature = false; //!< Whether the most recent temperature command has been a heat-and-wait command (M109) or not (M104).
		Temperature initial_temp = 0; //!< Temperature this nozzle needs to be at the start of the print.

		double retraction_e_amount_current = 0.0; //!< The current retracted amount (in mm or mm^3), or zero(i.e. false) if it is not currently retracted (positive values mean retracted amount, so negative impact on E values)
		double retraction_e_amount_at_e_start = 0.0; //!< The ExtruderTrainAttributes::retraction_amount_current value at E0, i.e. the offset (in mm or mm^3) from E0 to the situation where the filament is at the tip of the nozzle.

		double prime_volume = 0.0; //!< Amount of material (in mm^3) to be primed after an unretration (due to oozing and/or coasting)
		Velocity last_retraction_prime_speed = 0.0; //!< The last prime speed (in mm/s) of the to-be-primed amount

		double last_e_value_after_wipe = 0.0; //!< The current material amount extruded since last wipe

		unsigned fan_number = 0; // nozzle print cooling fan number
	...
	double current_e_offset = 0; //!< Offset to compensate for flow rate (mm or mm^3)
	double max_extrusion_offset = 0; //!< 0 to turn it off, normally 4
	double extrusion_offset_factor = 0; //!< default 1
	...
	*/
	...
	size_t extruder_base = 0;
	char extruder_char = 'A';
	bool extruder_info = false;

	size_t current_extruder;
	double current_fan_speed;
	unsigned fan_number; // current print cooling fan number
	static constexpr EGCodeFlavor flavor = EGCodeFlavor::TINA3D;
	...
	static constexpr bool is_volumetric = false;
	static constexpr bool relative_extrusion = false; //!< whether to use relative extrusion distances rather than absolute
...
public:
	...
	*/
	...
	void writeExtruderSelect(size_t extruder);
