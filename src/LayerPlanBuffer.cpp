...
void LayerPlanBuffer::addConnectingTravelMove(LayerPlan* prev_layer, const LayerPlan* newest_layer)
{
	...
	if(!prev_layer->last_planned_position || *prev_layer->last_planned_position != first_location_new_layer)
	{
		...
		const Settings& extruder_settings = Application::getInstance().scene.extruders[prev_layer->extruder_plans.back().extruder_nr].settings;
...
Preheat::WarmUpResult LayerPlanBuffer::computeStandbyTempPlan(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx)
{
	...
	Settings& extruder_settings = Application::getInstance().scene.extruders[extruder].settings;
...
{
	if(!Application::getInstance().scene.extruders[extruder_nr].settings.get<bool>("machine_nozzle_temp_enabled"))
		return;
...
void LayerPlanBuffer::insertPreheatCommand_multiExtrusion(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx)
{
	...
	const Settings& extruder_settings = Application::getInstance().scene.extruders[extruder].settings;
...
{   
	...
	if(prev_extruder != extruder)
	{
		...
		const Settings& previous_extruder_settings = Application::getInstance().scene.extruders[prev_extruder].settings;
	...
	else if(Application::getInstance().scene.extruders[extruder].settings.get<bool>("machine_extruders_share_heater"))
	{
	...
	for(size_t extruder_plan_idx = 0; extruder_plan_idx < layer_plan.extruder_plans.size(); extruder_plan_idx++)
	{
		...
		if(buffer.size() == 1 && extruder_plan_idx == 0)
		{
			...
			for(size_t extruder_idx = 0; extruder_idx < scene.extruders.size(); extruder_idx++)
			{
				...
				const Settings& other_extruder_settings = Application::getInstance().scene.extruders[extruder_idx].settings;
