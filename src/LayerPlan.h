...
public:
	...
	bool mode_skip_agressive_merge = false; //!< Wheter to give every new path the 'skip_agressive_merge_hint' property (see GCodePath); default is false.
...
private:
	...
	std::string current_mesh = "NONMESH"; //<! A unique ID for the mesh of the last planned move.
...
private:
	...
	*/
	GCodePath* getLatestPathWithConfig(const GCodePathConfig& config, SpaceFillType space_fill_type, const Ratio flow = 1.0_r, const Ratio speed_factor = 1.0_r);
...
public:
	...
	int getLayerNr() const { return layer_nr; }
	...
	*/
	void setMesh(const std::string mesh_id) { current_mesh = mesh_id; }

	void setGroup(size_t group_id) { current_group = group_id; }
	...
	*/
	void addExtrusionMove(Point p, const GCodePathConfig& config, SpaceFillType space_fill_type, const Ratio& flow = 1.0_r, Ratio speed_factor = 1.0_r, double fan_speed = GCodePathConfig::FAN_SPEED_DEFAULT);
	...
	*/
	void addPolygon(ConstPolygonRef polygon, int startIdx, const GCodePathConfig& config, WallOverlapComputation* wall_overlap_computation = nullptr, coord_t wall_0_wipe_dist = 0, const Ratio& flow_ratio = 1.0_r, bool always_retract = false);
	...
	*/
	void addPolygonsByOptimizer(const Polygons& polygons, const GCodePathConfig& config, WallOverlapComputation* wall_overlap_computation = nullptr, const ZSeamConfig& z_seam_config = ZSeamConfig(), coord_t wall_0_wipe_dist = 0, const Ratio flow_ratio = 1.0_r, bool always_retract = false, bool reverse_order = false);
