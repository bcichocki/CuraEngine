...
void SkirtBrim::getFirstLayerOutline(SliceDataStorage& storage, const size_t primary_line_count, const bool is_skirt, Polygons& first_layer_outline)
{
    ...
    const ExtruderTrain& support_infill_extruder = Application::getInstance().scene.current_mesh_group->settings.get<ExtruderTrain&>("support_infill_extruder_nr");
    const bool external_only = is_skirt || mesh_layer_group0.brim_outside_only; //Whether to include holes or not. Skirt doesn't have any holes.
    ...
    { // add brim underneath support by removing support where there's brim around the model
        ...
        if (storage.support.generated && primary_line_count > 0 && !storage.support.supportLayers.empty())
        {
            ...
            if (mesh_layer_group0.brim_replaces_support)
            {
                ...
                //  |+-+|     |+--+|
                //  +---+     +----+
                const coord_t primary_extruder_skirt_brim_line_width = mesh_layer_group0.skirt_brim_line_width * train.settings.get<Ratio>("initial_layer_line_width_factor");
...
{
    const auto& mesh_layer_group0 = Application::getInstance().scene.current_mesh_group->layer_group0;
    const Settings& adhesion_settings = Application::getInstance().scene.current_mesh_group->settings.get<ExtruderTrain&>("adhesion_extruder_nr").settings;
    const coord_t primary_extruder_skirt_brim_line_width = mesh_layer_group0.skirt_brim_line_width * adhesion_settings.get<Ratio>("initial_layer_line_width_factor");
...
void SkirtBrim::generate(SliceDataStorage& storage, Polygons first_layer_outline, const coord_t start_distance, const size_t primary_line_count, const bool allow_helpers /*= true*/)
{
    ...
    const coord_t primary_extruder_skirt_brim_line_width = mesh_layer_group0.skirt_brim_line_width * adhesion_settings.get<Ratio>("initial_layer_line_width_factor");
    const coord_t primary_extruder_minimal_length = mesh_layer_group0.skirt_brim_minimal_length;
    ...
    if (allow_helpers && mesh_layer_group0.support_brim_enable)
    {
    ...
    { // process other extruders' brim/skirt (as one brim line around the old brim)
        ...
        for (size_t extruder_nr = 0; extruder_nr < Application::getInstance().scene.extruders.size(); extruder_nr++)
        {
            ...
            const coord_t width = mesh_layer_group0.skirt_brim_line_width * train.settings.get<Ratio>("initial_layer_line_width_factor");
            const coord_t minimal_length = mesh_layer_group0.skirt_brim_minimal_length;
...
void SkirtBrim::generateSupportBrim(SliceDataStorage& storage)
{
    ...
    const coord_t brim_line_width = mesh_layer_group0.skirt_brim_line_width * support_infill_extruder.settings.get<Ratio>("initial_layer_line_width_factor");
    size_t line_count = mesh_layer_group0.support_brim_line_count;
    const coord_t minimal_length = mesh_layer_group0.skirt_brim_minimal_length;
